...
class GameState:
    ...
    initial_train_done: bool = False          # 초반 훈련 1회 완료 플래그
...
def decide(S: GameState, M: GameMap, P: Paths, turn: int) -> Actions:
    ...
    if not S.early_expansion_done:
        ...
        # 목표별 전사 배정 (3번째 목표는 새로 훈련된 전사만 사용)
        for i, target in enumerate(targets):
            ...
            if i < 2:
                ...
                # 3번째 목표는 초기 3명을 제외한(훈련병) 전사만 이동
                available = [w for w in idle if can_move_now(w) and w.id.num > START_WARRIORS]
    ...
    if len(my_bases) < MAX_BASES:
        ...
        for i, target in enumerate(S.expansion_targets):
            ...
            if i == 2:
                available = [w for w in available if w.id.num > START_WARRIORS]
            if available and total_cost + MOVE_COST <= S.gold:
                w = min(available, key=lambda w: P.hop[w.region][target])
                add_move(w.id, target, MOVE_COST)
                moved_ids.add(w.id)
